 ...
 * bar menu as a SEPARATE order (never bundled into the class payment). Shows
 ...
 * the order and creates a bar-only link (create_cafe_payment_link). Used by
...
export async function sendCafeMenuOffer(args: {
  ...
  try {
    ...
    else console.error(`Bar menu offer failed for client ${args.clientId} (non-blocking):`, err);
  }
}
