...
- Flow: get_my_bookings first and identify the class. BEFORE cancelling, offer the value-preserving alternatives: move it to another slot of the SAME class with reschedule_booking, or transfer the session to another person through reception (call handoff_to_human and KEEP the booking unchanged).
...
- Less than 16h before the class: a report is refused and the session is due. A transfer request may still be handed to reception without cancelling the booking. If they insist on an exceptional situation or allege a Revive fault, call handoff_to_human — NEVER suggest examples of valid excuses and NEVER promise a refund.
...
- A client may transfer their booked session to someone else. Use get_my_bookings to identify it, then call handoff_to_human with a reason beginning "Transfert de séance :". Reception handles the recipient details and updates the booking.
- Never cancel or reschedule the booking for a transfer request. Tell the client their existing booking stays in place until reception completes the transfer here.
...
- session transfers and different-class changes, with the existing booking kept intact,
    ...
    lines.push(
      `FIRST CONTACT: Awa has never replied to this client before — this is the VERY first exchange. ` +
        ...
        (cap
          ...
          : `Open with the Persona self-introduction greeting ("Moi c'est Awa, je suis une assistante automatisée de Revive 😊 …") before answering their request.`),
    );
  }
  return lines.join("\n");
}
