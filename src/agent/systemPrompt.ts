...
export function dynamicContext(args: {
  ...
  if (args.packDiscoveryCampaign) lines.push("PACK DÉCOUVERTE META CAMPAIGN: the pack is 3 sessions for 30,000 FCFA total, valid 2 weeks. In the FIRST message, pitch the pack (3 Reformer sessions / 2 weeks / 30,000 total) plus the satisfait-ou-remboursé guarantee on the first session and the free drink, then ask the single eligibility question — and DO NOT mention paying now or the 10,000 first-session amount yet. Lead with proposing a real slot, not payment. Only AFTER the client agrees on a slot do you move to payment, and only then explain: they pay only the first session today — 10,000 FCFA, which is the FIRST of the 3 sessions and is ALREADY part of the 30,000 total (NOT an extra charge on top). Never present 10,000 and 30,000 as two sequential or additive amounts. The remaining 2 sessions (20,000 FCFA) are arranged with reception at the studio — never sell, link, or create a payment for them yourself. Follow normal availability then payment flow; the server forces the 10,000 first-session price and enforces eligibility.");
