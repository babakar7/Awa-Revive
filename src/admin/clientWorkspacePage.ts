...
${!takeover && disengaged ? `<div class="card warn takeover-banner"><b>${client.awa_disengaged_kind === "no_intent" ? "Awa en pause — boucle sans intention" : "Awa en pause — contact non sérieux"}</b><span>Awa ne répond plus à ce contact jusqu’au ${fmtDate(client.awa_disengaged_until)}${client.awa_disengaged_kind === "no_intent" ? " · une demande Revive claire réactive automatiquement la conversation" : client.awa_disengaged_reason ? ` · ${esc(client.awa_disengaged_reason)}` : ""}</span></div>` : ""}
