...
<div class="card">
  <div class="task-list">
    ...
    ${L.late + L.kitchenFailed + L.clientFailed ? `<article class="task-item"><span class="task-priority danger" aria-hidden="true"></span><div class="task-copy"><b>Livraisons à surveiller</b><p>${[L.late ? `${L.late} en retard` : "", L.kitchenFailed ? `${L.kitchenFailed} cuisine non notifiée` : "", L.clientFailed ? `${L.clientFailed} destinataire non prévenu` : ""].filter(Boolean).join(" · ")}</p><div class="task-meta"><span class="muted">${L.open} commande(s) ouverte(s)</span></div></div><div class="task-action"><a class="act act--ghost act--sm" href="/admin/livraisons">Voir les livraisons</a></div></article>` : ""}
  </div>
