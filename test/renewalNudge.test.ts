...
const renewable = new Set(["plan_monthly"]);
...
describe("renewalNudgeCandidates", () => {
  ...
  it("NEVER nudges a one-time pack, even when it ends within the window", () => {
    ...
    const out = renewalNudgeCandidates([pack], now, 3, renewable);
  ...
  it("keeps a recurring order ending exactly at now and exactly at the horizon (inclusive)", () => {
    ...
    const out = renewalNudgeCandidates([atNow, atHorizon], now, 3, renewable);
    expect(out.map((c) => c.orderId).sort()).toEqual(["a", "b"]);
  });

  it("drops an order ending beyond the window", () => {
    const out = renewalNudgeCandidates([order({ endDate: "2026-07-20T10:00:00Z" })], now, 3, renewable);
    expect(out).toHaveLength(0);
  });

  it("drops an order that already ended (in the past)", () => {
    const out = renewalNudgeCandidates([order({ endDate: "2026-07-10T10:00:00Z" })], now, 3, renewable);
    expect(out).toHaveLength(0);
  });
  ...
  it("skips valid-until-cancelled plans (no endDate) and orders missing ids", () => {
    ...
    const out = renewalNudgeCandidates([noEnd, noContact, noId, noPlan], now, 3, renewable);
    expect(out).toHaveLength(0);
  });

  it("falls back to a default plan name when Wix returns none", () => {
    const out = renewalNudgeCandidates([order({ planName: undefined, endDate: "2026-07-13T10:00:00Z" })], now, 3, renewable);
